...
class EditorService {
    ...
    fun getEditors(category: String, country: String): List<Editor>? {
        val result = service.getEditors(category, country).execute()
        val sourceNewsApiResponse = result.body()
        return sourceNewsApiResponse?.sources
    }
